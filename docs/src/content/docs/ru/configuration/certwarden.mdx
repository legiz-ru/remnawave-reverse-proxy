...
5. В поле `Subject (and Common Name)` вводим домен, на который будем получать wildcard сертификаты (например, `*.example.com`)
...
7. Внизу появится окно ACME Orders
...
:::note
Аналогично ставится и на другие сервера, которые будут получать сертификаты
:::
...
10. Теперь мы можем их смонтировать в наш docker-compose.yml
