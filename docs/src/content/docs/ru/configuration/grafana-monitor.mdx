...
**[kutovoys](../../contribution/resources#kutovoys)**  
...
:::danger[Важно]
Выпуск сертификатов для вашего домена в этой статье не рассматривается. Сертификаты домена перед запуском docker-compose надо положить в __`/opt/monitoring/nginx`__, или прокинуть нужный volume где лежат сертификаты, например в __`/etc/letsencrypt/live/yourdomain.com`__
:::
