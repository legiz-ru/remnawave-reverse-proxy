...
5. Добавляем в наш nginx.conf блок server и upstream для Beszel
    ...
    :::danger[Важно]
    В данном конфиге используется сертификат `Wildcard`. Если у вас сертификат `ACME`, то не забудьте его сгенерировать для нового домена `beszel.example.com` (пример) и прописать его путь в `nginx.conf` и `docker-compose.yml` по пути в `/opt/remnawave`
    ::: 
