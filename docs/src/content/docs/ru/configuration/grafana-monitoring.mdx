...
:::caution{icon="setting"}
Если вы используете `remnawave-nginx`, то этапы ниже не являются обязательными.  
Установленный через скрипт eGames'а, `remnawave-nginx` уже включает в себя настроенный Nginx и SSL-сертификаты, отредактируйте существующий файл конфигурации в директории.
:::
