...
Netbird is an open-source platform based on WireGuard that allows you to easily create secure private networks. In this guide, we will walk through how to install and configure Netbird to link two servers so they can communicate securely with each other.
...
4. Fill in the fields (email, password, name) and confirm your registration via the email you receive.
...
3. Enter a name for the key (e.g., RemnawaveKey).
...
7. Save your key.
    ...
    A window will appear with the generated key (e.g., 12B653C1-4E03-452E-9DA9-40523493205B).
    Copy the key and save it in a safe place for future use.

</Steps>

## Installing Netbird on the Panel Server
...
In the Dashboard, find the Netbird IP address of the panel (in our case, it's 100.88.38.121; yours may be different).
...
## Configuring Netbird in the Remnawave Panel
