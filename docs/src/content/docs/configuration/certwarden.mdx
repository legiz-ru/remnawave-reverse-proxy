...
## Certwarden Client Installation and Configuration

:::note
We will install the client on the same server as the Remnawave panel, so we can also receive certificates.
:::

:::note
Analogously, it is installed on other servers that will receive certificates
:::
...
10. Now we can mount them in our docker-compose.yml
