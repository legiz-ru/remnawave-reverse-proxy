...
If you are using `remnawave-nginx`, the steps below are not mandatory.  
Installed via the eGames script, `remnawave-nginx` already includes a configured Nginx and SSL certificates, edit the existing configuration file in the directory.
:::
...
server {
    ...
    # Set cookie if user authenticates via link
    add_header Set-Cookie $set_cookie_header;

    # Redirect from main domain to the desired public dashboard in Grafana
    location = / {
