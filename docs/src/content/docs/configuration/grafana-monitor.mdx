...
Автор: **[pluralplay](../../contribution/resources#pluralplay)**  
...
:::danger[Важно]
Выпуск сертификатов для вашего домена в этой статье не рассматривается. Сертификаты домена перед запуском docker-compose надо положить в __/opt/monitoring/nginx__, или прокинуть нужный volume где лежат сертификаты, например в __/etc/letsencrypt/live/`yourdomain.com`__
:::
...
2. Создаем директории и docker-compose.yml 
    ...
    ```yaml title="docker-compose.yml" "x.x.x.x" "yourdomain.com" "subscription_link"
    services:
      vmsingle:
        ...
        networks:
          - grafana-network
        ports:
          - x.x.x.x:8428:8428 # Тут указываем NetBird IP (в данном случае сервера Grafana)
...
8. Создаем файл службы vmagent
    ...
    ```
    :::note[Внимание]
    В строке -remoteWrite.url=http://x.x.x.x:8428/api/v1/write надо заменить на IP Netbird (в данном случае сервера Grafana)
    :::
    
    Вставляем следующее содержимое:

    ```ini title="vmagent.service" "x.x.x.x"
    [Unit]
    ...
    ExecStart=/opt/monitoring/vmagent/vmagent \
          ...
          -remoteWrite.url=http://x.x.x.x:8428/api/v1/write # Тут указываем NetBird IP (в данном случае сервера Grafana)
    Restart=always
    RestartSec=5
...
3. Попадаем в главное меню Grafana
    ...
    ![Grafana main menu](https://raw.githubusercontent.com/eGamesAPI/remnawave-reverse-proxy/refs/heads/main/docs/src/assets/ru/netbird/peer_netbird_node.png)
...
6. В секции Connection в поле `URL` вводим `http://vmsingle:8428`
    ...
    ![Grafana data source](https://raw.githubusercontent.com/eGamesAPI/remnawave-reverse-proxy/refs/heads/main/docs/src/assets/ru/netbird/peer_netbird_node.png)
...
4. Где victoriametrics выбираем наш prometheus
   ...
   ![Grafana Dashboard](https://raw.githubusercontent.com/eGamesAPI/remnawave-reverse-proxy/refs/heads/main/docs/src/assets/ru/netbird/peer_netbird_node.png)
